<%page args="src, caption='PDF - smellið til að opna', img_width=512, img_height=None, rz_op='fit_width', css_class='pdf-img'" />
<%!
import os
import subprocess
import hashlib
...
%>
% if caption:
  ${ figure.body(thumb, caption=caption, width=img_width, height=img_height, alt=orig_src, resize=False, css_class="${ css_class }", img_link=orig_src, link_target="_blank") }
% else:
  <a href="${ orig_src }" target="_blank" title="PDF - smellið til að opna"><img src="${ thumb }" loading="lazy" alt="${ orig_src |h }" class="${ css_class }"></a>
% endif
